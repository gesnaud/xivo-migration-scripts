...
if [ -n "${MIGRATION}" ]; then
  ...
  UPGRD_DONE=/var/lib/pf-fai/upgrade_lenny_done
...
if ! pkg_installed pf-fai; then
  ...
  apt-get update >/dev/null
  apt-get -y install pf-fai >/dev/null
...
apt-get update >/dev/null
apt-get -y install pf-fai-xivo-1.1-gallifrey >/dev/null
apt-get update >/dev/null
