...
if [ -n "${MIGRATION}" ]; then
  ...
  if ! pkg_installed pf-fai; then
    ...
    SL_BAK="/etc/apt/sources.list.inst-bak"
    CLEANUP_FILES="${CLEANUP_FILES} ${SL_BAK}"
    # Don't overwrite backup from unfinished installation
    if [ ! -e ${SL_BAK} ]; then
      cp -a /etc/apt/sources.list ${SL_BAK}
    fi
...
aptitude -y purge dhcp-client dhcp3-client
...
aptitude update >/dev/null
aptitude -y install pf-fai-xivo-1.1-gallifrey >/dev/null
