...
OLD_XIVO_BRANCH="XIVO K-9"
XIVO_BRANCH="XIVO Dalek"
...
if [ -n "${MIGRATION}" ]; then
  ...
  BACKUP_CONF=${BACKUP_CONF_BASE}/k-9-dalek
...
wget -q -O - http://mirror.xivo.io/d-i/etch/pkg.cfg | debconf-set-selections
wget -q -O - http://mirror.xivo.io/d-i/etch/classes/xivo-dalek/custom.cfg | debconf-set-selections
...
aptitude update >/dev/null
aptitude -y install pf-fai-xivo-1.0-dalek >/dev/null
